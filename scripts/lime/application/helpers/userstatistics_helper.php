...
//auto detect if english
        ...
        //Pie Chart
            ...
            while (isset ($gdata[$i])) {
                ...
                if ($gdata[$i] == 0 || (is_object($sLanguageCode) && strlen(substr($aHelperArray[$i], 0, strlen($sLanguageCode->gT("Comments")))) >= 0)) {
					// $sQuestionType == "O" && 
...
//auto detect if english
			...
			for ($i = 0; $i < count($lblout); $i++)
			{
				if (stristr($lblout[$i], "Comments") != false)
				{
					array_splice($gdata, $i, 1);
					array_splice($lblout, $i, 1);
				}
			}
